...
**Consent — ask once.** Sharing is opt-in and remembered. Run the command below; if
it prints `CONSENT_REQUIRED` and exits non-zero, the user has not chosen yet — **stop
and ask** whether to share a **redacted** copy of this session: **Always** (remember,
never ask again), **Just this once**, or **Never**. Do not send until they answer,
then re-run appending their choice: `--consent always`, `--consent once`, or
`--consent never`. If instead it prints `CONSENT=never … sharing is off` (exit 0),
just tell them sharing is off and stop — do not re-ask.
