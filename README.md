...
Use this when an app needs a backend: database schema, reactive queries, mutations, server functions, auth-aware data access, real-time features, file storage, scheduled jobs, mobile/web app backends, hosted frontend integration planning, static frontend hosting on Convex, public live deployments, or production scaling guidance.
...
The app exposes tools for starting Convex apps, adding Convex to existing JavaScript and TypeScript projects, planning hosted frontend + Convex integrations, and getting Convex scaling guidance. The plugin skills also guide Codex through moving a static frontend onto the Convex static-hosting component and deploying it live.
...
My hosted dashboard needs realtime comments, files, and workspace users.
Move this static app onto Convex hosting and deploy it live.
...
- `skills/` - Codex routing guidance for backend and scaling tasks
